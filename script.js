...
(() => {
  ...
  let options = [];
  let colors = [
    "#f94144","#f3722c","#f9844a","#f9c74f","#90be6d","#43aa8b","#577590",
    "#277da1","#9b5de5","#f15bb5","#00bbf9","#ffd166"
  ];
  ...
  function spin() {
    ...
    const speed = parseInt(speedRange.value, 10) || 6;
    const minSpins = Math.max(3, speed - 1); // base spins
    const extra = Math.floor(Math.random() * 3); // randomness
    const spins = minSpins + extra + 2; // ensure nice rotation
    ...
    function animate(now) {
      ...
      if (t < 1) {
        ...
        resultEl.textContent = "👉 Resultado: " + options[selected];
      }
    }
    requestAnimationFrame(animate);
  }
  ...
  saveDefaultBtn.addEventListener("click", () => {
    saveToLocal("ruleta.options", options);
    alert("Opciones guardadas como defecto en este navegador.");
  });
